...
from Kynan.modules.sql import antiflood_sql as sql
    ...
    if match:
        ...
        try:
            ...
            update.effective_message.edit_text(
                f"Di unmute oleh {mention_html(user.id, html.escape(user.first_name))}.",
                parse_mode="HTML",
            )
        except:
            pass
...
def flood(update, context):
    ...
    if limit == 0:
        if conn:
            text = msg.reply_text(
                "Saya tidak menerapkan pengendalian flood di {}!".format(chat_name),
            )
        else:
