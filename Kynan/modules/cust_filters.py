...
from Kynan import dispatcher, LOGGER, DRAGONS
...
from Kynan.modules.helper_funcs.misc import build_keyboard_parser
from Kynan.modules.helper_funcs.msg_types import get_filter_type
...
from Kynan.modules.sql import cust_filters_sql as sql
...
def rmall_filters(update, context):
    ...
    if member.status != "creator" and user.id not in DRAGONS:
        ...
        update.effective_message.reply_text(
            f"Apakah Anda yakin ingin menghentikan SEMUA filter di? {chat.title}? Tindakan ini tidak bisa dibatalkan.",
...
__help__ = """
ᐉ /filters*:* Daftar semua filter aktif yang disimpan dalam obrolan.
➣ *Hanya admin:*
ᐉ /filter <katakunci> <membalas pesan>*:* Tambahkan filter ke obrolan ini. Bot sekarang akan membalas pesan itu setiap kali 'kata kunci''\
disebutkan. Jika Anda membalas stiker dengan kata kunci, bot akan membalas dengan stiker itu. CATATAN: semua filter \
kata kunci dalam huruf kecil. Jika Anda ingin kata kunci Anda menjadi kalimat, gunakan tanda kutip. misalnya: /filter Rexa Ganteng
 Pisahkan balasan yang berbeda dengan `%%%` untuk mendapatkan balasan acak
➣ *Contoh:* 
 `/filter "nama filter"
 Membalas 1
 %%%
...
ᐉ /stop <nama filter>*:* Hentikan filter itu.

➣ *Hanya pembuat obrolan:*

ᐉ /removeallfilters*:* Hapus semua filter obrolan sekaligus.
➣ *Catatan*: Filter juga mendukung format MARKDOWN seperti: {first}, {last} dll. dan tombol.
