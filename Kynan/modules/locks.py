...
import Kynan.modules.sql.locks_sql as sql
from Kynan import dispatcher, DRAGONS, LOGGER
        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                if conn:
                    ...
                    chat_id = update.effective_chat.id
                    chat_name = update.effective_message.chat.title
                    text = "Tidak terkunci {} untuk semua orang!".format(ltype)
                ...
                if conn:
                    ...
                    chat_id = conn
                    chat_name = chat.title
                    text = "Tidak terkunci {} untuk semua orang di {}!".format(ltype, chat_name)
                else:
                    if update.effective_message.chat.type == "private":
                        send_message(
                            update.effective_message,
                ...
                send_message(
                    update.effective_message,
                    "Apa yang Anda coba buka ...? Coba /locktypes untuk daftar yang dapat dikunci.",
                )

        else:
            send_message(update.effective_message, "Apa yang Anda coba buka?...?")
            ...
            if lockable == "bots":
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        ...
                        send_message(
                            update.effective_message,
                            "Hanya admin yang diperbolehkan menambahkan bot di chat ini! Keluar dari sini.",
                        )
                        break
            else:
                try:
...
Anda berada di bot yang tepat!
Modul kunci memungkinkan Anda untuk mengunci beberapa item umum di \
dunia telegram; bot akan secara otomatis menghapusnya!
 ᐉ /locktypes*:* Daftar semua kemungkinan tipe kunci
 
➣ *Admins only:*
 ᐉ /lock <type>*:* Kunci item dari jenis tertentu (tidak tersedia secara pribadi)
 ᐉ /unlock <type>*:* Buka kunci item dari jenis tertentu (tidak tersedia secara pribadi)
 ,
 ᐉ /locks*:* Daftar kunci saat ini dalam obrolan ini.
 
➣ Kunci dapat digunakan untuk membatasi pengguna grup.
misalnya:
Mengunci url akan otomatis menghapus semua pesan dengan url, stiker pengunci akan membatasi semua \
pengguna non-admin dari mengirim stiker, dll.
Mengunci bot akan menghentikan non-admin menambahkan bot ke obrolan.
➣ *Note:*
 ➣ Izin membuka kunci *info* akan mengizinkan anggota (non-admins) untuk mengubah informasi grup, seperti deskripsi atau nama grupe
 ➣ Izin membuka kunci *pin* akan mendukung anggota (non-admin) untuk menyematkan pesan dalam grup
"""
