...
from Kynan import LOGGER, TEMP_DOWNLOAD_DIRECTORY, telethn as bot
...
async def handler(event):
    ...
    if not reply_message.media:
        await event.reply("```Balas ke gambar/stiker.```")
        return
    file = await bot.download_media(reply_message)
    msg = await event.reply("```Mencoret coret gambar ini! (」ﾟﾛﾟ)｣ ```")
    text = str(event.pattern_match.group(1)).strip()
    if len(text) < 1:
        return await msg.edit("Anda mungkin ingin mencoba `/mmf teks`")
...
async def drawText(image_path, text):
    ...
    if os.name == "nt":
        ...
        fnt = "./Kynan/resources/default.ttf"
