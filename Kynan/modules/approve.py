...
from Kynan import dispatcher, DRAGONS
from Kynan.modules.helper_funcs.extraction import extract_user
from telegram.ext import CallbackContext, CallbackQueryHandler
import Kynan.modules.sql.approve_sql as sql
...
def approve(update, context):
    ...
    if member.status in ("administrator", "creator"):
        message.reply_text(
            "Pengguna sudah menjadi admin - kunci, daftar blokir, dan antibanjir tidak berlaku untuk mereka.",
        )
        return ""
...
def unapproveall_btn(update: Update, context: CallbackContext):
    ...
    if query.data == "unapproveall_user":
        if member.status == "creator" or query.from_user.id in DRAGONS:
            ...
            message.edit_text("Berhasil Membatalkan persetujuan semua pengguna dalam Obrolan ini.")
            return
...
➣ *Admin commands:*
ᐉ /approval*:* Periksa status persetujuan pengguna di obrolan ini.
ᐉ /approve*:* Menyetujui pengguna. Kunci, daftar hitam, dan antibanjir tidak akan berlaku lagi.
ᐉ /unapprove*:* Tidak disetujui pengguna. Mereka sekarang akan dikenakan kunci, daftar hitam, dan antiflood lagi.
ᐉ /approved*:* Daftar semua pengguna yang disetujui.
ᐉ /unapproveall*:* Unapprove *ALL* pengguna dalam obrolan. Ini tidak dapat dibatalkan.
"""
