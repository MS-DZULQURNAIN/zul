...
from Kynan import pbot as app, BOT_ID
from Kynan.utils.errors import capture_err
from Kynan.utils.permissions import adminsOnly
from Kynan.ex_plugins.dbfunctions import (
    alpha_to_int,
    get_karma,
    get_karmas,
    int_to_alpha,
...
from Kynan.utils.filter_groups import karma_negative_group, karma_positive_group
...
async def upvote(_, message):
    ...
    await message.reply_text(
        f"Karma Bertambah {user_mention} Oleh 1 \nTotal Poin: {karma}"
    )
...
async def downvote(_, message):
    ...
    await update_karma(chat_id, await int_to_alpha(user_id), new_karma)
    await message.reply_text(
        f"Karma Dikurangi {user_mention} By 1 \nTotal Points: {karma}"
    )
...
async def karma(_, message):
    ...
    if not message.reply_to_message:
        m = await message.reply_text("Menganalisis Karma... Akan Memakan Waktu 10 Detik")
...
async def captcha_state(_, message):
    ...
    if state == "on":
        ...
        await message.reply_text("Sistem karma diaktifkan.")
    elif state == "off":
        karma_off(chat_id)
        await message.reply_text("Sistem karma dinonaktifkan.")
    else:
