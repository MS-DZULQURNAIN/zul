...
import Kynan.modules.sql.feds_sql as sql
from Kynan import (
    EVENT_LOGS,
    LOGGER,
    SUPPORT_CHAT,
...
from Kynan.modules.helper_funcs.alternate import send_message
...
from Kynan.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == "":
        ...
        if not x:
            update.effective_message.reply_text(
                f"Tidak bisa bersekutu! Mohon hubungi @{SUPPORT_CHAT} jika masalah tetap ada.",
            )
            return
...
def fed_owner_help(update: Update, context: CallbackContext):
    update.effective_message.reply_text(
        """*➣ Fed Owner Only:*
 ᐉ `/newfed <fed_name>`*:* Creates a Federation, One allowed per user
 ᐉ `/renamefed <fed_id> <new_fed_name>`*:* Renames the fed id to a new name
 ᐉ `/delfed <fed_id>`*:* Delete a Federation, and any information related to it. Will not cancel blocked users
 ᐉ `/fpromote <user>`*:* Assigns the user as a federation admin. Enables all commands for the user under `Fed Admins`
 ᐉ `/fdemote <user>`*:* Drops the User from the admin Federation to a normal User
 ᐉ `/subfed <fed_id>`*:* Subscribes to a given fed ID, bans from that subscribed fed will also happen in your fed
 ᐉ `/unsubfed <fed_id>`*:* Unsubscribes to a given fed ID
 ᐉ `/setfedlog <fed_id>`*:* Sets the group as a fed log report base for the federation
 ᐉ `/unsetfedlog <fed_id>`*:* Removed the group as a fed log report base for the federation
 ᐉ `/fbroadcast <message>`*:* Broadcasts a messages to all groups that have joined your fed
 ᐉ `/fedsubs`*:* Shows the feds your group is subscribed to `(broken rn)`""",
        parse_mode=ParseMode.MARKDOWN,
    )
...
def fed_admin_help(update: Update, context: CallbackContext):
    update.effective_message.reply_text(
        """*➣ Fed Admins:*
 ᐉ `/fban <user> <reason>`*:* Fed bans a user
 ᐉ `/unfban <user> <reason>`*:* Removes a user from a fed ban
 ᐉ `/fedinfo <fed_id>`*:* Information about the specified Federation
 ᐉ `/joinfed <fed_id>`*:* Join the current chat to the Federation. Only chat owners can do this. Every chat can only be in one Federation
 ᐉ `/leavefed <fed_id>`*:* Leave the Federation given. Only chat owners can do this
 ᐉ `/setfrules <rules>`*:* Arrange Federation rules
 ᐉ `/fedadmins`*:* Show Federation admin
 ᐉ `/fbanlist`*:* Displays all users who are victimized at the Federation at this time
 ᐉ `/fedchats`*:* Get all the chats that are connected in the Federation
 ᐉ `/chatfed `*:* See the Federation in the current chat\n""",
        parse_mode=ParseMode.MARKDOWN,
    )
...
ᐉ /fbanstat*:* Shows if you/or the user you are replying to or their username is fbanned somewhere or not
ᐉ /fednotif <on/off>*:* Federation settings not in PM when there are users who are fbaned/unfbanned
ᐉ /frules*:* See Federation regulations\n""",
        parse_mode=ParseMode.MARKDOWN,
    )
