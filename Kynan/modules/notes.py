...
import Kynan.modules.sql.notes_sql as sql
from Kynan import LOGGER, JOIN_LOGGER, SUPPORT_CHAT, dispatcher, DRAGONS
from Kynan.modules.disable import DisableAbleCommandHandler
from Kynan.modules.helper_funcs.handlers import MessageHandlerChecker
from Kynan.modules.helper_funcs.chat_status import user_admin, connection_status
from Kynan.modules.helper_funcs.misc import build_keyboard, revert_buttons
from Kynan.modules.helper_funcs.msg_types import get_note_type
from Kynan.modules.helper_funcs.string_handling import (
...
def cmd_get(update: Update, context: CallbackContext):
    ...
    if len(args) >= 2 and args[1].lower() == "noformat":
        ...
        update.effective_message.reply_text("masukan nama note atau lihat semua notes yang tersedia tekan /notes")
...
def save(update: Update, context: CallbackContext):
    ...
    if data_type is None:
        msg.reply_text("bro, catatan itu ga ada")
        return
    ...
    if member.status != "creator" and user.id not in DRAGONS:
        ...
        buttons = InlineKeyboardMarkup(
            [
                ...
                [InlineKeyboardButton(text="Batalkan", callback_data="notes_cancel")],
            ],
        )
        update.effective_message.reply_text(
...
def list_notes(update: Update, context: CallbackContext):
    ...
    msg = "Dapatkan catatan dengan `/nomor catatan` or `#namacatatan` \n\n  *ID*    *Note* \n"
...
__help__ = """
➣ *Notes.*
ᐉ /get <notename>*:* dapatkan catatan dengan nama catatan ini
ᐉ #<notename>*:* sama dengan /get
ᐉ /notes or /saved*:* daftar semua catatan yang disimpan dalam obrolan ini
ᐉ /number *:* Akan menarik catatan nomor itu dalam daftar
...
*Hanya admin:*
ᐉ /save <notename> <notedata>*:* menyimpan notedata sebagai catatan dengan nama notename
Tombol dapat ditambahkan ke catatan dengan menggunakan sintaks tautan penurunan harga standar - tautan hanya boleh diawali dengan a \
`buttonurl:` bagian, seperti: `[katakata](buttonurl:example.com)`. Periksa `/markdownhelp` untuk info lebih lanjut
ᐉ /save <notename>*:* simpan pesan balasan sebagai catatan dengan nama notename
 Pisahkan balasan yang berbeda menurut `%%%` untuk mendapatkan catatan acak
 *Contoh:*
 `/save nama catatan
 Membalas 1
 %%%
 ...
 %%%
 Membalas 3`
ᐉ /clear <notename>*:* hapus catatan dengan nama ini
ᐉ /removeallnotes*:* menghapus semua catatan dari grup
