...
from Kynan import ubot2
from Kynan.events import register
from Kynan import telethn as tbot, TEMP_DOWNLOAD_DIRECTORY, SUPPORT_CHAT
...
async def _(fry):
    ...
    if not fry.reply_to_msg_id:
        await kntl.edit("`balas ke stiker`")
        return
    ...
    if reply_message.sender.bot:
        await fry.edit("`Balas sticker untuk dihancurkan`")
        return
    ...
    async with ubot2.conversation(chat) as conv:
        try:
            ...
            await fry.reply(f"`Kesalahan, ceritakan masalahnya pada @{SUPPORT_CHAT}`")
            return
        if response.text.startswith("Forward"):
            await fry.edit(f"`Kesalahan, ceritakan masalahnya pada @{SUPPORT_CHAT}`")
        else:
