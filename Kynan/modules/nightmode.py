import os
from Kynan.modules.sql.night_mode_sql import (
    add_nightmode,
    rmnightmode,
    get_all_chat_id,
    is_nightmode_indb,
...
from Kynan import telethn as tbot, OWNER_ID
...
@register(pattern="^/(nm|Nightmode|NightMode|NM) ?(.*)")
async def profanity(event):
    ...
    if "on" in input:
        if event.is_group:
            ...
            await event.reply("Mode Malam diaktifkan untuk obrolan ini.")
    if "off" in input:
        if event.is_group:
            if not is_nightmode_indb(str(event.chat_id)):
                    await event.reply(
                        "Mode Malam Sudah Mati untuk Obrolan ini"
                    )
                    return
        rmnightmode(str(event.chat_id))
        await event.reply("NightMode Disabled!")
    if not "off" in input and not "on" in input:
        await event.reply("Harap Tentukan on atau off!")
        return
    ...
    for pro in chats:
        try:
            await tbot.send_message(
              int(pro.chat_id), "12:00, Grup Tutup Hingga 06:00. Mode Malam Dimulai ! \n**Powered By [˹ҡʏɴλɴ ꭙ ꝛᴏʙᴏᴛ˼༗](t.me/KynanUserbot)**"
            )
            await tbot(
...
async def job_open():
    ...
    for pro in chats:
        try:
            await tbot.send_message(
              int(pro.chat_id), "06:00, Grup Dibuka.\n**Powered By [˹ҡʏɴλɴ ꭙ ꝛᴏʙᴏᴛ˼༗](t.me/KynanUserbot)**"
            )
            await tbot(
            functions.messages.EditChatDefaultBannedRightsRequest(
...
ᐉ /nm <on/off> *:* Untuk Mengaktifkan Mode Malam.
"""
